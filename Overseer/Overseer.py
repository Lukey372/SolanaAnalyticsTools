...
async def checkcreator(ctx, url:str):
    ...
    await ctx.send(embed=address_embed, hidden=True)
...
async def tps(ctx):
    ...
    await ctx.send(embed=tps_embed, hidden=True)
...
async def lmnft(ctx, url:str):
    try:
        ...
        await ctx.send(embed=info_embed, hidden=True)
        print(f"Overseer checked {url}")
...
async def start_raffle_watch(ctx):
    await ctx.send(content="Starting raffle watcher...", hidden=True)
    try:
        while True:
...
async def pumpwatch5m(ctx):
    await ctx.send(content="Starting 5m Pumpwatch...", hidden=True)
    try:
        while True:
...
async def pumpwatch10m(ctx):
    await ctx.send(content="Starting 10m Pumpwatch...", hidden=True)
    try:
        while True:
            collections_info = requests.get("https://stats-mainnet.magiceden.io/collection_stats/popular_collections/sol?limit=20")
            collections_info_json = collections_info.json()
            top5_min_volume = collections_info_json['top15m']
...
@slash.slash(description="Sends daily mints from SOLDecoder, requires bearer token.")
@commands.has_permissions(administrator=True)
